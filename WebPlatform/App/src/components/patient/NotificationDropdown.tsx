...
const isValidNotificationType = (type: string): boolean => {
  const validTypes = [
    'appointment', 'appointment_confirmed', 'appointment_cancelled',
    'lab_results', 'lab_result_ready', 'lab_request_created',
    'lab_request_accepted', 'lab_request_rejected',
    'new_message', 'doctor_response',
    'new_follow_up', 'appointment_confirmed', 'cancelled_appointment',
    'new_appointment', 'nurse_assignment', 'nurse_unassignment'
  ];
...
const NotificationDropdown = ({ isOpen, onClose, unreadCount }: NotificationDropdownProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const cached = localStorage.getItem('patient_notifications');
      if (cached) {
        try {
  ...
  const handleMarkAllRead = useCallback(async () => {
    try {
      ...
      // Emit event for parent component to update unread count
  ...
  const handleNotificationClick = useCallback(async (notification: Notification) => {
    ...
    const typeRoute: Record<string, string> = {
      ...
      lab_result_ready: '/patient/lab-results',
      lab_request_created: '/patient/lab-results',
      lab_request_accepted: '/patient/lab-results',
      lab_request_rejected: '/patient/lab-results',
      new_message: '/patient/messages',
      doctor_response: '/patient/messages',
      new_follow_up: '/patient/follow-up-requests',
    };

    const route = typeRoute[notification.type];
    if (route) {
      // For appointment_confirmed, set flag to auto-select confirmed tab
      if (notification.type === 'appointment_confirmed') {
        sessionStorage.setItem('scrollToTab', 'confirmed');
      }
      
      // Mark as read first, then navigate
      if (!notification.is_read) {
        await handleMarkAsRead(notification).catch(err => {
